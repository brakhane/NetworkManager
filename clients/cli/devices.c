...
static char**
lldp_format_management_addresses (NMLldpNeighbor *neighbor)
{
...
static void
show_device_lldp_list (NMDevice *device, NmCli *nmc, const char *fields_str)
{
	gs_free_error GError *error = NULL;
	GPtrArray *neighbors;
	guint i;
...
do_device_lldp_list (NmCli *nmc, int argc, char **argv)
{
	...
	if (device) {
		show_device_lldp_list (device, nmc, fields_str);
	} else {
		NMDevice **devices = nmc_get_devices_sorted (nmc->client);
		int i;

		for (i = 0; devices[i]; i++)
			show_device_lldp_list (devices[i], nmc, fields_str);
